...
export function CarDetailClient({ car }: { car: any }) {
  ...
  const galleryImages = [
    {
      src: car.image,
      alt: `Tampilan eksterior ${car.name}`,
      label: "Eksterior",
    },
    {
      src: car.gallery?.interior || "https://placehold.co/600x400.png",
      alt: `Tampilan interior ${car.name}`,
      label: "Interior",
    },
    {
      src: car.gallery?.front || "https://placehold.co/600x400.png",
      alt: `Tampilan depan ${car.name}`,
      label: "Depan",
    },
    {
      src: car.gallery?.back || "https://placehold.co/600x400.png",
      alt: `Tampilan belakang ${car.name}`,
      label: "Belakang",
    },
  ];
        ...
        <div className="grid lg:grid-cols-5 gap-8 md:gap-12 animate-fade-in-up">
              ...
              <div className="grid grid-cols-4 gap-4">
                {galleryImages.map((image, index) => (
                  <button
                    ...
                    className={`relative aspect-video w-full overflow-hidden rounded-lg transition-all duration-200 ${
                      activeImage === index
          ...
          <div className="lg:col-span-2">
            <Card className="bg-card/50 border-border/20 p-6 md:p-8 rounded-2xl lg:sticky lg:top-28">
              ...
              <Button
                size="lg"
                className="w-full bg-primary text-primary-foreground hover:bg-primary/90 rounded-full font-bold text-lg py-7 mt-4"
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 text-base">
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/10">
                  ...
                  <span>
                    BBM: <strong>{car.fuel}</strong>
                  </span>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/10">
                  <Users className="h-5 w-5 text-primary" />
                  <span>
                    Kapasitas: <strong>{car.capacity} Orang</strong>
                  </span>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/10">
                  <GitCommitHorizontal className="h-5 w-5 text-primary" />
                  <span>
                    Transmisi: <strong>{car.transmission}</strong>
                  </span>
                </div>
              </div>
            </div>
