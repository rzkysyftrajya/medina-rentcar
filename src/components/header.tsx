...
import { Menu, X, Car, LayoutList, GalleryHorizontal, Info, FileQuestion, Mail, FileText, Home } from 'lucide-react';
...
export function Header() {
  const navItems = [
    ...
    { href: '/layanan', label: 'Layanan', icon: <LayoutList className="h-6 w-6" /> },
    { href: '/galeri', label: 'Galeri', icon: <GalleryHorizontal className="h-6 w-6" /> },
    { href: '/tentang', label: 'Tentang', icon: <Info className="h-6 w-6" /> },
    { href: '/faq', label: 'FAQ', icon: <FileQuestion className="h-6 w-6" /> },
    { href: '/kontak', label: 'Kontak', icon: <Mail className="h-6 w-6" />, desktopHidden: true },
    { href: '/syarat-ketentuan', label: 'Syarat & Ketentuan', icon: <FileText className="h-6 w-6" />, desktopHidden: true },
  ];
        ...
        <div className="container mx-auto flex h-24 items-center justify-between gap-4 px-6">
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-2">
              <Image src="/logo.png" alt="Medina Rentcar Logo" width={160} height={40} className="h-10 w-auto" priority />
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center justify-end flex-1 gap-4">
             <nav className="flex items-center justify-center bg-card/50 px-4 py-2 rounded-full border border-border/20 shadow-sm">
                {navItems.filter(item => !item.desktopHidden).map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={cn(
              ...
              <Button asChild className="rounded-full font-bold whitespace-nowrap">
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">Booking via WA</a>
              </Button>
          </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <motion.div 
              ...
              exit={{ x: '100%', transition: { duration: 0.3, ease: 'circIn' } }}
            >
              ...
              >
                  ...
                  <Button asChild size="lg" className="w-full rounded-full font-bold text-lg py-7">
                      <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">Booking via WA</a>
                  </Button>
              </motion.div>
            </motion.div>
