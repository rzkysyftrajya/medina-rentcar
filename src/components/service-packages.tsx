...
export function ServicePackages() {
  const generateWhatsappUrl = (packageName: string) => {
    return `https://wa.me/6282277757234?text=${encodeURIComponent(`Halo Medina Rentcar, saya tertarik dengan ${packageName}.`)}`;
  }
  
  return (
    <div className="w-full max-w-4xl mx-auto">
      <Accordion type="single" defaultValue="lepasKunci" collapsible className="w-full">
        {packages.map((pkg) => (
          <AccordionItem key={pkg.id} value={pkg.id} className="border-none mb-4">
            <AccordionTrigger className="flex justify-between items-center w-full p-6 rounded-2xl cursor-pointer transition-all duration-300 hover:no-underline bg-card/50 hover:bg-card/80 border border-border/20 data-[state=open]:bg-primary data-[state=open]:text-primary-foreground data-[state=open]:shadow-lg data-[state=open]:shadow-primary/30">
              <span className="text-xl md:text-2xl font-bold text-left">{pkg.name}</span>
              <span className="text-lg md:text-xl font-semibold text-right">{pkg.price}</span>
            </AccordionTrigger>
            <AccordionContent className="mt-2 p-8 bg-card/80 rounded-b-2xl border-x border-b border-border/20">
              <div className="grid md:grid-cols-3 gap-8 items-center">
                <div className="md:col-span-2">
                  <h3 className="text-2xl font-bold text-primary mb-4">Detail Paket</h3>
                ...
                <div className="text-center md:text-right">
                  <Button asChild size="lg" className="w-full md:w-auto rounded-full font-bold bg-primary hover:bg-primary/90">
                    <a href={generateWhatsappUrl(pkg.name)} target="_blank">Pesan Paket Ini</a>
                  </Button>
                </div>
              </div>
            </AccordionContent>
