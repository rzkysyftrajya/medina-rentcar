...
const generateItemListSchema = (cars: any[]) => {
  return {
    ...
    "description": "Daftar mobil yang tersedia untuk disewa di Medina Rentcar Medan.",
    "itemListElement": cars.map((car, index) => {
      const getPrice = (prices: any) => {
         if (prices.matic) return parseFloat(prices.matic.replace(/[^0-9]/g, ''));
         if (prices.manual) return parseFloat(prices.manual.replace(/[^0-9]/g, ''));
         return 0;
      }
      ...
      return {
        ...
        "item": {
          ...
          "brand": {
            "@type": "Brand",
            "name": car.name.split(' ')[0] // e.g., Toyota
          },
          "modelDate": car.year.split('-')[0], // Use first year in range
...
function ArmadaPageContent() {
    ...
    const filteredCars = useMemo(() => {
        return carData.filter(car => {
            ...
            const transmissionMatch = filters.transmission === 'all' || 
                                      (filters.transmission === 'Manual' && car.transmission.includes('Manual')) ||
                                      (filters.transmission === 'Matic' && car.transmission.includes('Matic'));
            ...
            <div className="bg-background">
                <div className="container mx-auto px-6 pt-24 pb-12 sm:pt-32">
                    <div className="text-center mb-12 animate-fade-in-up">
                        <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground">Jelajahi Armada Kami</h1>
                        <p className="text-muted-foreground mt-4 max-w-2xl mx-auto">Gunakan filter untuk menemukan mobil yang paling sesuai dengan kebutuhan Anda.</p>
                    </div>
                    ...
                    {filteredCars.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredCars.map((car, index) => (
                                <div key={car.slug} className="animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                                    <FleetCard car={car} />
                                </div>
                            ))}
                        </div>
                       ...
                       <div className="text-center animate-fade-in-up" style={{ animationDelay: '400ms' }}>
                          <Alert className="max-w-md mx-auto bg-card/50">
                            ...
                            <AlertDescription>
                              Maaf, tidak ada mobil yang cocok dengan kriteria pencarian Anda. Coba ubah atau reset filter.
                            </AlertDescription>
                          </Alert>
                       </div>
                    )}
